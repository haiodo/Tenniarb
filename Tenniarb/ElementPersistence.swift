...
extension Element {
    ...
    static func buildItemData(_ item: DiagramItem, _ itemBlock: TennNode, addPos: Bool) {
        ...
        if nx || ny || addPos {
            itemBlock.add(
                TennNode.newCommand(
                    PersistenceItemKind.Position.commandName, TennNode.newFloatNode(Double(item.x)), TennNode.newFloatNode(Double(item.y))))
        }
...
extension Element {
    ...
    static func parseLinkData(
        _ link: DiagramItem, _ cmdName: String, _ blChild: TennNode, _ sourceIndex: inout Int, _ targetIndex: inout Int
    ) {
        switch cmdName {
        case PersistenceItemKind.Description.commandName, "desription":
