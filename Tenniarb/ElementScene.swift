...
open class ItemDrawable: Drawable, @unchecked Sendable {
...
open class DrawableContainer: ItemDrawable, @unchecked Sendable {
...
class DrawableStyle: @unchecked Sendable {
...
class DrawableLineStyle: DrawableStyle, @unchecked Sendable {
...
class DrawableItemStyle: DrawableStyle, @unchecked Sendable {
...
class SceneStyle: DrawableStyle, @unchecked Sendable {
...
open class DrawableScene: DrawableContainer, @unchecked Sendable {
    ...
    init(
        _ element: Element, darkMode: Bool, executionContext: ExecutionContextEvaluator?, scaleFactor: CGFloat = 1,
        buildChildren: Bool = true, items: [DiagramItem]? = nil
    ) {
                    ...
                    if let line = de as? DrawableLine {
                        ...
                        self.activeDrawables.append(
                            SelectorBox(
                                ...
                                size: CGSize(
                                    width: (editBoxBounds?.width ?? deBounds.width) + 10,
                                    height: (editBoxBounds?.height ?? deBounds.height) + 10),
    ...
    fileprivate func buildRoundRect(
        _ bounds: CGRect, _ style: DrawableItemStyle, _ e: DiagramItem, _ textBox: TextBox, _ elementDrawable: DrawableContainer,
        fill: Bool = true, stack: Int = 0
    ) {
        let rectBox = RoundBox(
            bounds: bounds,
            style, fill: fill)
        rectBox.stack = stack
    ...
    fileprivate func buildCircle(
        _ bounds: CGRect, _ style: DrawableStyle, _ e: DiagramItem, _ textBox: TextBox, _ elementDrawable: DrawableContainer,
        fill: Bool = true
    ) {
        let rectBox = CircleBox(
            bounds: bounds,
            style, fill: fill)
        rectBox.append(textBox)
    ...
    fileprivate func buildEmptyRect(
        _ bounds: CGRect, _ style: DrawableStyle, _ e: DiagramItem, _ textBox: TextBox, _ elementDrawable: DrawableContainer
    ) {
    ...
    fileprivate func buildInvisibleRect(
        _ bounds: CGRect, _ style: DrawableStyle, _ e: DiagramItem, _ textBox: TextBox, _ elementDrawable: DrawableContainer
    ) {
    ...
    static func calculateSize(attrStr: NSAttributedString) -> CGSize {
        ...
        let frameSize = CTFramesetterSuggestFrameSizeWithConstraints(
            fs, CFRangeMake(0, attrStr.length), nil, CGSize(width: 30000, height: 30000), nil)
        ...
        if let lines = CTFrameGetLines(frame) as? [CTLine] {
            ...
            for l in lines {
                ...
                for i in 0..<range.length {
                    if let attr = attrStr.attribute(NSAttributedString.Key.font, at: range.location + i, effectiveRange: nil),
                        let font = attr as? NSFont
                    {
                    ...
                    if let attr = attrStr.attribute(NSAttributedString.Key.paragraphStyle, at: range.location + i, effectiveRange: nil),
                        let ps = attr as? NSParagraphStyle
                    {
    ...
    static func toAttributedString(
        tokens: [MarkdownToken], font: NSFont, color: CGColor, shift: inout CGPoint, imageProvider: ImageProvider, layout: [TextPosition],
        ...
        return MarkDownAttributedPrinter.toAttributedStr(
            tokens, font: font, paragraphStyle: textStyle, foregroundColor: NSColor(cgColor: color)!, shift: &shift,
    ...
    func buildItemDrawable(_ e: DiagramItem, _ elementDrawable: DrawableContainer) {
        ...
        if let bodyNode = properties.get("body") {
            ...
            bodyAttrString =
                DrawableScene.toAttributedString(
                    tokens: MarkdownLexer.getTokens(code: (titleValue.count > 0 ? "\n" : "") + prepareBodyText(textValue)),
                    font: NSFont.systemFont(ofSize: bodyStyle.fontSize), color: bodyStyle.textColor, shift: &shift,
                    imageProvider: imageProvider, layout: [vertical, horizontal], lineSpacing: style.lineSpacing)
        }
        ...
        let attrString = DrawableScene.toAttributedString(
            tokens: titleTokens, font: NSFont.systemFont(ofSize: style.fontSize), color: style.textColor, shift: &shift,
            imageProvider: imageProvider, layout: [horizontal, vertical])
        ...
        var finalTextBounds = CGRect(
            origin: CGPoint(x: offx, y: offy), size: CGSize(width: textBounds.width + shift.x, height: textBounds.height + shift.y))
...
public class RoundBox: DrawableContainer, @unchecked Sendable {
    ...
    func doDraw(_ context: CGContext, at point: CGPoint) {
        ...
        if self.stack > 0 {
            for i in 1...self.stack {
                ...
                context.translateBy(
                    x: self.stackStep.x * CGFloat(self.stack - i),
...
public class EmptyBox: DrawableContainer, @unchecked Sendable {
...
class InvisibleBox: EmptyBox, @unchecked Sendable {
...
public class DrawableLine: ItemDrawable, @unchecked Sendable {
    ...
    func addLabel(_ label: String, imageProvider: ImageProvider) {
        var shift = CGPoint(x: 0, y: 0)
        let attrStr = DrawableScene.toAttributedString(
            tokens: MarkdownLexer.getTokens(code: label.replacingOccurrences(of: "\\n", with: "\n")),
            font: NSFont.systemFont(ofSize: self.style.fontSize), color: self.style.borderColor, shift: &shift,
            imageProvider: imageProvider, layout: [.Center, .Middle])
    ...
    public override func draw(context: CGContext, at point: CGPoint) {
        ...
        if drawArrow {
            ...
            var ept =
                extraPoints.count > 0
                ? CGPoint(x: extraPoints[extraPoints.count - 1].x + point.x, y: extraPoints[extraPoints.count - 1].y + point.y) : fromPt
    ...
    public override func getBounds() -> CGRect {
        ...
        return CGRect(x: minX, y: minY, width: abs(maxX - minX), height: max(abs(maxY - minY), 5.0)).insetBy(
            dx: -1 * style.lineWidth, dy: -1 * style.lineWidth)
    }
    public override func getSelectorBounds() -> CGRect {
        var minX = min(source.x, target.x)
        var maxX = max(source.x, target.x)
...
public class SelectorLine: ItemDrawable, @unchecked Sendable {
    var source: CGPoint
    var target: CGPoint
    var color: CGColor
    var extra: [CGPoint]
...
public class CircleBox: DrawableContainer, @unchecked Sendable {
    var style: DrawableStyle
    var fill: Bool = true
    ...
    public override func draw(context: CGContext, at point: CGPoint) {
        ...
        let clipBounds = CGRect(
            origin: CGPoint(x: bounds.origin.x + point.x + 2, y: bounds.origin.y + point.y + 2),
            size: CGSize(width: bounds.size.width - 4, height: bounds.size.height - 4))
