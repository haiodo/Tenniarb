...
class TennTextView: NSTextView {
    ...
    func drawAnnotations() {
            ...
            let context = NSGraphicsContext.current?.cgContext
        {
            ...
            var lineNumber =
                newLineRegex.numberOfMatches(in: self.string, options: [], range: NSMakeRange(0, firstVisibleGlyphCharacterIndex)) + 1
            ...
            while glyphIndexForStringLine < NSMaxRange(visibleGlyphRange) {
                ...
                let glyphRangeForStringLine = layoutManager.glyphRange(
                    forCharacterRange: characterRangeForStringLine, actualCharacterRange: nil)
                ...
                while glyphIndexForGlyphLine < NSMaxRange(glyphRangeForStringLine) {
                    ...
                    let lineRect = layoutManager.lineFragmentUsedRect(
                        forGlyphAt: glyphIndexForGlyphLine, effectiveRange: &effectiveRange, withoutAdditionalLayout: true)
...
class TextPropertiesDelegate: NSObject, NSTextViewDelegate, NSTextDelegate, IElementModelListener {
    ...
    fileprivate func updateAnnotations() {
        ...
        // Snapshot taken on MainActor; detached task only reads them back through
        // ExecutionContext.getEvaluated, which is guarded by its own syncQueue.
        let ctx = self.controller.elementStore?.executionContext
        let di = self.diagramItem
        let element = self.element
        nonisolated(unsafe) let drawable = di.flatMap { self.controller.scene.scene?.drawables[$0] }
        let rootCtx = ctx?.rootCtx
        Task.detached(priority: .background) { @Sendable [weak self] in
            let p = TennParser()
            let currentNode = p.parse(currentText)
            var newExpressionLines: [Int: String] = [:]
            if let ctx = ctx {
                var evaluated: [TennToken: JSValue] = [:]
                if let di = di {
                    evaluated = ctx.getEvaluated(di, currentNode, drawable)
                } else if di == nil, let ictx = rootCtx, ictx.element == element {
                    evaluated = ctx.getEvaluated(element!, currentNode)
                }
                for (k, v) in evaluated {
                    newExpressionLines[k.line] = v.toString()
                }
            }

            let result = newExpressionLines
            await MainActor.run {
                guard let self = self else { return }
                self.expressionLines = result
    ...
    func highlight() {
        ...
        while true {
            ...
            case .symbol:
                if tok.size > 0 {
                    attrs.append(
                        Attr(key: NSAttributedString.Key.foregroundColor, value: symbolColor, range: NSMakeRange(tok.pos, tok.size)))
                }
            case .stringLit:
