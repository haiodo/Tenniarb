        ...
        if let tok = self.token {
            ...
            case .IntLit, .FloatLit:
                result.append(
                    NSAttributedString(
                        string: tok.literal,
                        attributes: [NSAttributedString.Key.foregroundColor: numberColor, NSAttributedString.Key.font: font]))
            case .Ident:
                result.append(
                    NSAttributedString(
                        string: tok.literal,
                        attributes: [NSAttributedString.Key.foregroundColor: symbolColor, NSAttributedString.Key.font: font]))
            case .StringLit:
                if clean {
                    result.append(
                        NSAttributedString(
                            string: tok.literal,
                            attributes: [NSAttributedString.Key.foregroundColor: stringColor, NSAttributedString.Key.font: font]))
                } else {
                    result.append(
                        NSAttributedString(
                            string: "\"\(quote(tok.literal))\"",
                            attributes: [NSAttributedString.Key.foregroundColor: stringColor, NSAttributedString.Key.font: font]))
                }
            case .Expression:
                result.append(
                    NSAttributedString(
                        string: "$(\(tok.literal))",
                        attributes: [NSAttributedString.Key.foregroundColor: expressionColor, NSAttributedString.Key.font: font]))
            case .Image:
                ...
                if let dta = Data(base64Encoded: tok.literal, options: .ignoreUnknownCharacters) {
                    ...
                    if let sz = img?.size {
                        ...
                        image1Attachment.bounds = CGRect(
                            x: 0, y: font.capHeight / 2 + -1 * bb.height / 2, width: bb.width, height: bb.height)
                    }
                }
