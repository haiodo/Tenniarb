...
public class Element: Identifiable, @unchecked Sendable {
    ...
    func clone(cloneItems: Bool = true, cloneElement: Bool = true) -> Element {
        ...
        if cloneItems {
            ...
            cloneEl.items.append(
                contentsOf: self.items.map({ (itm) in
                    let copy = itm.clone()
                    copy.parent = cloneEl
                    itemsMap[itm] = copy
        ...
        if cloneElement {
            cloneEl.elements.append(
                contentsOf: self.elements.map({
                    (el) in
 ...
 */
public class ElementModel: Element, @unchecked Sendable {
...
public class DiagramItem: @unchecked Sendable {
    var kind: ItemKind
    var name: String
    var id: UUID
    var parent: Element?
...
class LinkItem: DiagramItem, @unchecked Sendable {
