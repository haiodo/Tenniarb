...
fileprivate func calculateValue(
    _ node: TennNode?,
    _ currentContext: JSContext,
...
@objc public class UtilsContext: NSObject, UtilsProtocol {
    ...
    func textSize(_ text: String, _ fontSize: Any?) -> [Double] {
        ...
        let frameSize = CTFramesetterSuggestFrameSizeWithConstraints(
            fs, CFRangeMake(0, attrString.length), nil, CGSize(width: 1000, height: 1000), nil)
...
@objc public class ItemContext: NSObject {
    ...
    fileprivate func registerByTag() {
        let byTag: @convention(block) (String) -> Any? = { tagName in
            ...
            return self.parentCtx.element.items.map({ (e) -> ItemContext? in self.parentCtx.itemsMap[e] }).filter({ e in
                e != nil && e!.itemObject[tagName] != nil
            }).map { e in e!.itemObject }
        }
    ...
    fileprivate func updateGetContext(
        _ node: TennNode?, newItems: inout [String: Any], newEvaluated: inout [TennToken: JSValue], _ drawable: Drawable?
    ) -> Bool {
        cleanContext()
...
public class ElementContext: NSObject, @unchecked Sendable {
    var element: Element
    ...
    init(_ context: ExecutionContext, _ element: Element) {
        ...
        let scene = DrawableScene(
            self.element, darkMode: false, executionContext: self.context.evalContext, scaleFactor: self.context.scaleFactor,
            buildChildren: false)
...
public class ExecutionContextEval: @unchecked Sendable, ExecutionContextEvaluator {
    var context: ExecutionContext!
    init() {
    }
...
// Public entry points serialize through `syncQueue`; `*NoSync` variants are for reentrant
// calls already inside that queue. Not an actor: render paths call this synchronously.
public class ExecutionContext: @unchecked Sendable, ExecutionContextEvaluator {
    ...
    public func updateAll(_ notifier: @escaping @Sendable () -> Void) {
        DispatchQueue.global(qos: .utility).async(execute: {
            syncQueue.sync {
                if let root = self.rootCtx {
                    root.updateContext()
                    let scene = DrawableScene(
                        root.element, darkMode: false, executionContext: self.evalContext, scaleFactor: self.scaleFactor,
                        buildChildren: false)
