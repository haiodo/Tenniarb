...
class ExportManager: NSObject, NSMenuDelegate {
    ...
    func renderImage() -> (NSImage, CGRect) {
        ...
        let scene = DrawableScene(
            self.element!, darkMode: false, executionContext: self.viewController?.elementStore?.executionContext,
        ...
        if PreferenceConstants.preference.renderEnableBackground {
            context?.fill(
                CGRect(
                    x: 0, y: 0, width: Int(imgBounds.width * scaleFactor),
                    height:
    ...
    func generateHtml() -> String? {
        ...
        if let tiff = img.tiffRepresentation, let bitmapImage = NSBitmapImageRep(data: tiff) {
            if let base64Data = bitmapImage.representation(using: .png, properties: [:])?.base64EncodedString() {
                ...
                htmlContent +=
                    "\t\t<img style=\"border: 1px solid #eeeeee;\" width=\"\(bounds.width)\" height=\"\(bounds.height)\" src=\"data:image/png;base64,"
    ...
    func createMenu() -> NSMenu {
        ...
        for itm in self.exportTypes {
            ...
            menuItem.image = NSImage.init(
                size: NSSize(width: 24, height: 24), flipped: false,
                drawingHandler: {
                    (rect) in
                    img?.draw(in: rect)
                    return true
    ...
    func exportPdf() {
        if let element = self.element {
            ...
            let info = NSPDFInfo()
            panel.beginSheet(
                with: info, modalFor: nil,
                completionHandler: { (result) -> Void in
                    if result == 1 {
                        let scene = DrawableScene(
                            element, darkMode: false, executionContext: self.viewController?.elementStore?.executionContext)
